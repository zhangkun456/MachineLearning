...
# 构造K 就是要训练得到的值
K = tf.Variable(tf.random_normal(mean=0, shape=[2, 1]))
L = tf.Variable(tf.random_normal(mean=0, shape=[1, 1]))

print('k=' + str(K))
K1 = [K[0]]
K2 = [K[1]]
calcY = tf.add(tf.matmul(x_data, K1), K2)
...
for i in range(1000):
    ...
    if (i + 1) % 25 == 0:
        print('Step #' + str(i + 1) + ' K = ' + str(KValue)+' ------LValue='+str(tmp_loss))
...
for i in x_vals:
    n = n + 1
    best_fit.append(k * i + l)
    print('i=' + str(n) + ' x=' + str(i) + ' best_fit=' + str(k * i + l))
